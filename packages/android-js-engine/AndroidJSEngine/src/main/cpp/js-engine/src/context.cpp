...
#include "jigra-api/api_device.h"
#include "jigra-api/api_geolocation.h"
#include "jigra-api/api_kv.h"
#include "jigra-api/api_notifications.h"
#include "quickjs/cutils.h"

Context::Context(const std::string &name, JSRuntime *parent_rt, NativeInterface *native, JigraInterface *jig) {
  this->name = name;
  this->qjs_context = JS_NewContext(parent_rt);
  this->native_interface = native;
  this->jigra_interface = jig;
  ...
  if (this->jigra_interface != nullptr) {
    this->init_jigra_api();
  }
}
...
void Context::init_jigra_api() {
  this->init_jigra_device_api();
  this->init_jigra_geolocation_api();
  this->init_jigra_kv_api();
  this->init_jigra_notifications_api();
}

void Context::init_jigra_device_api() const {
  ...
  JS_SetPropertyStr(this->qjs_context, global_obj, "JigraDevice", device);

  JS_FreeValue(this->qjs_context, global_obj);
}

void Context::init_jigra_kv_api() const {
  ...
  JS_SetPropertyStr(this->qjs_context, global_obj, "JigraKV", kv);

  JS_FreeValue(this->qjs_context, global_obj);
}

void Context::init_jigra_notifications_api() const {
  ...
  JS_SetPropertyStr(this->qjs_context, global_obj, "JigraNotifications", notifications);

  JS_FreeValue(this->qjs_context, global_obj);
}

void Context::init_jigra_geolocation_api() const {
  ...
  JS_SetPropertyStr(this->qjs_context, global_obj, "JigraGeolocation", geolocation);
