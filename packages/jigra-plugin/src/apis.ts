...
export interface NotificationScheduleOptions {
   ...
   *
   * If set, this overrides the `smallIcon` option from Jigra
   * configuration.
   *
 ...
 * @since 1.0.0
 */
export interface JigraGeolocation {
  /**
 ...
 */
export interface JigraNotifications {
  /**
   * Schedule a local notification
   * @example `JigraNotifications.schedule()`
   * @param options
   * @returns void
   * @since 1.0.0
   */
 ...
 *
 * sendMessage, transferUserInfo and updateApplicationContext are raw routes to the WCSession delegate methods, but have no effects currently in a JigraWatch Watch application.
 * They could be used if a native watch app is developed as a companion app to a Jigra app
 */
export interface JigraWatch {
  /**
   * Sends a message to the watch with the sendMessage() WCSession delegate method
   *
   * This has no effect on a JigraWatch watch app
   */
...
export interface JigraAPI {
  JigraDevice: JigraDevice;
  JigraKV: JigraKV;
  JigraNotifications: JigraNotifications;
  JigraGeolocation: JigraGeolocation;
  JigraWatch: JigraWatch;
}
