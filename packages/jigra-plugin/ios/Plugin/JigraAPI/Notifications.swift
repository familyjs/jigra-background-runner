...
enum JigraNotificationsErrors: Error, Equatable {
...
struct NotificationOption {
    ...
    init(from dict: [String: Any?]) throws {
        guard let id = dict["id"] as? Int else {
            throw JigraNotificationsErrors.invalidOptions(reason: "notification id is required")
        }

        guard let title = dict["title"] as? String else {
            throw JigraNotificationsErrors.invalidOptions(reason: "notification title is required")
        }

        guard let body = dict["body"] as? String else {
            throw JigraNotificationsErrors.invalidOptions(reason: "notification body is required")
        }

        guard let scheduleAt = dict["scheduleAt"] as? Date else {
            throw JigraNotificationsErrors.invalidOptions(reason: "notification schedule date is required")
        }
...
@objc protocol JigraNotificationsExports: JSExport {
    static func schedule(_ options: JSValue)
}

class JigraNotifications: NSObject, JigraNotificationsExports {
    ...
    static func requestPermission() throws {
        var permissionsError: JigraNotificationsErrors?
        ...
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge]) { _, error in
            if let error = error {
                permissionsError = JigraNotificationsErrors.unknownError(reason: "\(error)")
            }
    ...
    static func schedule(_ options: JSValue) {
        do {
            if JigraNotifications.checkPermission() != "granted" {
                throw JigraNotificationsErrors.permissionDenied
            }

            if options.isUndefined || options.isNull {
                throw JigraNotificationsErrors.invalidOptions(reason: "options are null")
            }

            if !options.isArray {
                throw JigraNotificationsErrors.invalidOptions(reason: "options must be an array")
            }
            ...
            if let notificationOptions = options.toArray() as? [[String: Any?]] {
                for option in notificationOptions {
                    ...
                    if let extra = notificationOption.extra {
                        content.userInfo["jig_extra"] = extra
                    }
