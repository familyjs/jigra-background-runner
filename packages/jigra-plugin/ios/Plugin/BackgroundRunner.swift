...
public class BackgroundRunner {
    ...
    private func loadRunnerConfig() throws -> RunnerConfig? {
        guard let jigraConfigFileURL = Bundle.main.url(forResource: "jigra.config.json", withExtension: nil) else {
            throw BackgroundRunnerPluginError.runnerError(reason: "jigra.config.json file not found")
        }

        let jigraConfigFileData = try Data(contentsOf: jigraConfigFileURL)

        if let jigConfig = JSTypes.coerceDictionaryToJSObject(try JSONSerialization.jsonObject(with: jigraConfigFileData) as? [String: Any]) {
            if let jsonConfig = jigConfig[keyPath: "plugins.BackgroundRunner"] as? JSObject {
