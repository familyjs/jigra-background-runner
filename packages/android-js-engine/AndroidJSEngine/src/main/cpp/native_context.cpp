...
extern "C" JNIEXPORT jlong JNICALL Java_app_family_android_1js_1engine_Context_createRunnerContext(JNIEnv *env, jobject thiz, jlong runner_ptr, jstring name, jobject fml_api) {
  auto *runner = (Runner *)runner_ptr;

  auto *api_global = env->NewGlobalRef(fml_api);
  auto *fml_native = new NativeJigraInterface(env, api_global);

  return (jlong)(long)runner->create_context(env->GetStringUTFChars(name, nullptr), fml_native);
}
extern "C" JNIEXPORT void JNICALL Java_app_family_android_1js_1engine_Context_destroyRunnerContext(JNIEnv *env, jobject thiz, jlong runner_ptr, jlong ptr) {
  auto *runner = (Runner *)runner_ptr;
  auto *context = (Context *)ptr;

  runner->destroy_context(context->name);
}
extern "C" JNIEXPORT jstring JNICALL Java_app_family_android_1js_1engine_Context_evaluate(JNIEnv *env, jobject thiz, jlong ptr, jstring code, jboolean ret_value) {
  auto *context = (Context *)ptr;
...
extern "C" JNIEXPORT void JNICALL Java_app_family_android_1js_1engine_Context_registerGlobalFunction(JNIEnv *env, jobject thiz, jlong ptr, jstring function_name, jobject function) {
  auto *context = (Context *)ptr;
...
extern "C" JNIEXPORT void JNICALL Java_app_family_android_1js_1engine_Context_dispatchEvent(JNIEnv *env, jobject thiz, jlong ptr, jstring event, jstring details) {
  auto *context = (Context *)ptr;
