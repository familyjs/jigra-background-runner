...
@objc protocol JigraDeviceExports: JSExport {
    func getNetworkStatus() -> [String: Any]?
    func getBatteryStatus() -> [String: Any]
}

enum JigraDeviceErrors: Error, Equatable {
    case unknownError(reason: String)
}

class JigraDevice: NSObject, JigraDeviceExports {
    ...
    private func checkError() throws {
        if let err = initError {
            throw JigraDeviceErrors.unknownError(reason: "\(err)")
        }

        if reachability == nil {
            throw JigraDeviceErrors.unknownError(reason: "reachability failed to initialize")
        }
    }
...
fileprivate extension Reachability.Connection {
    var equivalentEnum: JigraDevice.Connection {
        switch self {
        case .unavailable:
            return .unavailable
        case .wifi:
...
extension JigraDevice.Connection {
