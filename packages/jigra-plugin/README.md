...
![Enable Background Mode Capability in Xcode](https://github.com/familyjs/jigra-background-runner/raw/main/docs/enable_background_mode_capability.png)
...
![Configure Background Modes in Xcode](https://github.com/familyjs/jigra-background-runner/raw/main/docs/configure_background_modes.png)

After enabling the Background Modes capability, add the following to your app's `AppDelegate.swift`:

At the top of the file, under `import Jigra` add:
...
Read about [Configuring `Info.plist`](https://jigrajs.web.app/docs/ios/configuration#configuring-infoplist) in the [iOS Guide](https://jigrajs.web.app/docs/ios) for more information on setting iOS permissions in Xcode
...
If you are upgrading from 1.0.0 with an existing Android project, be sure to delete the `android-js-engine-release.aar` from `android/src/main/libs`.
...
During the course of building complex applications, its sometimes necessary to perform work while the application is not in the foreground. The challenge with standard Jigra applications is that the webview is not available when these background events occur, requiring you to write native code to handle these events. This is where the Background Runner plugin comes in.
...
In addition to the standard Web APIs, Background Runner also supports a number of [custom Jigra APIs](#jigra-api) custom APIs that expose relevant mobile device functionality
...
Check permissions for the various Jigra device APIs.
...
sendMessage, transferUserInfo and updateApplicationContext are raw routes to the WCSession delegate methods, but have no effects currently in a <a href="#jigrawatch">JigraWatch</a> Watch application.
They could be used if a native watch app is developed as a companion app to a Jigra app

| Prop                           | Type                                                                     | Description                                                                                                                                                                       |
| ------------------------------ | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`sendMessage`**              | <code>(options: []) =&gt; void</code>                                    | Sends a message to the watch with the sendMessage() WCSession delegate method This has no effect on a <a href="#jigrawatch">JigraWatch</a> watch app                              |
| **`transferUserInfo`**         | <code>(options: []) =&gt; void</code>                                    | Sends information to the watch with the transferUserInfo() WCSession delegate method This has no effect on a <a href="#jigrawatch">JigraWatch</a> watch app                       |
| **`updateApplicationContext`** | <code>(options: []) =&gt; void</code>                                    | Updates the application context on the watch with the updateApplicationContext() WCSession delegate method This has no effect on a <a href="#jigrawatch">JigraWatch</a> watch app |
